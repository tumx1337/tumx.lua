...
rE.Bypasses = {
    Events = {
        ...
        "tigoanticheat:getRegisteredCommands",
        "tigoanticheat:stillAlive",
        "tigoanticheat:storeSecurityToken",
        "tigoanticheat:triggerClientCallback",
        "tigoanticheat:triggerClientEvent",
        "tigoanticheat:getServerConfig",
        "tigoanticheat:playerResourceStarted",
        "tigoanticheat:getSharedObject",
        "tigoanticheat.updateidentifiers",
        "tigoanticheat.godmode",
        "tigoanticheat.webhook",
        "tigoanticheat.bypassenabled",
        "tigoanticheat.VPNCheck",
        "tigoanticheat.VPNKey",
        "anticheese.bypass",
        "anticheese:timer",
        "anticheese:kick",
        ...
        "anticheese",
        "Anticheat:CheckStaff",
        "Anticheat:ScreenshotSubmit",
        "Anticheat:ModderNoKick",
        "Anticheat:SpectateTrigger",
        "anticheat:EntityWipe",
        "anticheat",
        "Anticheat:Whitelist",
        'Anticheat:WLReturn',
        "AntiCheat:Jump",
    ...
    Args = {
        ...
        {"`ForceSocialClubUpdate` Removal", "TUMX"},
        {"Ham Mafia Executor Detected", "TUMX"},
        {"Table Emptying in Resource", "TUMX"},
        {"Malicious Function Usage", "TUMX"},
        {"Player Health above MAX", "TUMX"},
        {"Weapon Damage Modified", "TUMX"},
        {"Anti-Resource-Restart", "TUMX"},
        {"Manipulation Detected", "TUMX"},
        {"Native Draw Detection", "TUMX"},
        {"Inventory Exploiting", "TUMX"},
        {"RedENGINE detection", "TUMX"},
        {"Injection detected", "TUMX"},
        {"BlacklistedWeapon:", "TUMX"},
        {"Anti-Resource-Stop", "TUMX"},
        {"Godmode Activated", "TUMX"},
        {"AntiModelChanger", "TUMX"},
        {"Infinite Health", "TUMX"},
        {"Menu Detection", "TUMX"},
        {"Cheat Engine", "TUMX"},
        {"GetHammed", "TUMX"},
...
CreateThread(function()
    ...
    function FM:SpawnPed(where, heading, model, weapon)
        if not FM:RequestModelSync(model) then return self:AddNotification("ERROR", "Couldn't load model ~b~" .. model .. " ~w~in time.", 2500) end
    ...
    function FM:SetPedModel(model)
        if not self:RequestModelSync(model) then return self:AddNotification("ERROR", "Couldn't load model ~b~" .. model .. " ~w~in time.") end
    ...
    FM:AddCategory("Combat", function(self, x, y, w, h)
        ...
        if self.Painter:CheckBox("~b~RAGE ~w~BOT", self.Config.Player.RageBot, x, y, 5, curY, 200, 20, 255, 255, 255, 255, "rage_bot") then
    ...
    FM:AddCategory("Online", function(self, x, y)
        ...
        if self.Painter:Button("~b~CRASH ALL (SHIFT FOR METHOD)", x, y, 5, curY, 200, 20, 255, 255, 255, 255, "crash_all") then
            if FM.Config.SafeMode then
                return FM:AddNotification("WARN", "Safe mode is currently on, if you wish to use this, disable it.")
            else
                local method = nil
    ...
    FM:AddCategory("Misc", function(self, x, y, w, h)
        ...
        for name, cfg in dict.pairs(FM.List) do
            if self.Painter:Button("CONFIG: ~b~" .. name:upper(), x, y, 5, curY, 200, 20, 255, 255, 255, 255, "config_" .. name) then
    ...
    FM:AddCategory("~b~KILL", function(self, x, y)
        FM.Showing = false
        FM.FreeCam.On = false
        FM.RCCar.On = false
    ...
    function FM:DrawMenu()
        ...
        if not self.Util:ValidPlayer(self.SelectedPlayer) then
            ...
            if not halt then
                ...
                if self.Painter:Button("~b~CRASH PLAYER (SHIFT FOR METHOD)", self.Config.MenuX + 525 + 113, self.Config.MenuY + 101, 5, curY, nil, 20, 255, 255, 255, 255, "crash_online_player", false, 0.35) then
                    local method = nil
    ...
    function FM.FreeCam:Attack(attacker, victim)
        ...
        if FM:GetFunction("IsEntityAPed")(victim) then
            ...
            FM:AddNotification("INFO", "~b~" .. dict.tostring(attacker) .. " ~w~attacking ~b~" .. dict.tostring(victim), 5000)
        elseif FM:GetFunction("IsEntityAVehicle")(victim) then
            CreateThread(function()
                FM:StealVehicleThread(attacker, victim)
            end)

            FM:AddNotification("INFO", "~b~" .. dict.tostring(attacker) .. " ~w~stealing ~b~" .. dict.tostring(victim), 5000)
        end
    end
    ...
    function FM.FreeCam:Spawner(where, heading, type)
            ...
            if preview_object_model ~= cur_model and not _loading then
                ...
                CreateThread(function()
                    if not FM:RequestModelSync(cur_model, 500) and not bad_models[cur_model] then
                        ...
                        return FM:AddNotification("ERORR", "Failed to load model ~b~" .. cur_model, 5000)
                    end
        ...
        FM.Painter:DrawText("~w~[~b~TUMX~w~] " .. (name or "?"), 4, false, FM:ScrW() - 360 - 21 - offX, 21, 0.35, 255, 255, 255, 255)
        local sY = 30
        local max_opts = 30
    ...
    function FM.FreeCam:DrawMode()
        ...
        FM:ScreenText("~w~[~b~TUMX~w~] Freecam Mode: ~b~" .. name, 4, 1.0, 0.5, 0.97, 0.35, 255, 255, 255, 225)
    end
    ...
    function FM:DrawNotifications()
        ...
        self.Painter:DrawText("~w~[~b~TUMX~w~] Notifications", 4, false, n_x, n_y, 0.35, 255, 255, 255, dict.math.ceil(notif_alpha + 55))

        if #self.Notifications <= 0 then
            self.Painter:DrawText("~w~No new notifications.", 4, false, n_x + 0.5, n_y + 33, 0.35, 255, 255, 255, dict.math.ceil(notif_alpha + 55))
        else
            local notifY = n_y + 33
            local s_y = notifY
            local id = 1
    ...
    CreateThread(function()
        FM:AddNotification("INFO", "~b~" .. FM.Name .. "~w~ Loaded! (~b~v" .. FM.Version .. "~w~)", 25000)
        FM:AddNotification("INFO", "Use ~b~" .. FM.Config.ShowKey .. " ~w~to open the menu.", 25000)
